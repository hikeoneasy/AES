...
#define GETWORD(pt) (((word)(pt)[0] << 24) ^ ((word)(pt)[1] << 16) ^ ((word)(pt)[2] << 8) ^ ((word)(pt)[3]))
#define PUTWORD(ct, st) {(ct)[0] = (byte)( (st) >> 24 ); (ct)[1] = (byte)( (st) >> 16 ); (ct)[2] = (byte)( (st) >> 8); (ct)[3] = (byte)(st);}

byte GF_xtime(byte x)
{
	return ((((x >> 7) & 0x1) == 1) ? (x << 1) ^ 0x1b : x << 1);
}
void AES32_Enc_Table_generation() {

	word Te0[256], Te1[256], Te2[256], Te3[256], Te4[256];
	byte sx;
	word value;
...
word RotWord(word w32) {
	return (w32 << 8) | (w32 >> 24);
}

//=====
// 키스케줄용: (x0, x1, x2, x3) --> (S[x0], S[x1], S[x2], S[x3])
word SubWord(word w32) {
	return (SBox[w32 >> 24] << 24) ^ (SBox[(w32 >> 16) & 0xff] << 16) ^ (SBox[(w32 >> 8) & 0xff] << 8) ^ SBox[w32 & 0xff];
}


void AES32_Enc_KeySchedule(byte k[16], word rk[11][4]) {
	word tmp;
	int rcon_counter;

	for (int i = 0; i < 4; i++) {
		rk[0][i] = GETWORD(k + 4 * i);
	}
	rcon_counter = 0;
	for (int i = 1; i < 11; i++) {
		tmp = rk[i - 1][3];
...
void State2byte(word state[4], byte b[16])
{
	...
	PUTWORD(b+4, state[1]);
	PUTWORD(b+8, state[2]);
	PUTWORD(b+12, state[3]);
}

void AES32_round(word state[4], word rk[4])
{
	word tmp[4];
...
void AES32_Encrypt(byte pt[16], word rk[11][4], byte ct[16])
{
	word state[4], tmp[4];
	...
	tmp[0] = (Te4[state[0] >> 24] & 0xff000000) ^ (Te4[(state[1] >> 16) & 0xff] & 0x00ff0000) ^ (Te4[(state[2] >> 8) & 0xff] & 0x0000ff00) ^ (Te4[state[3] & 0xff] & 0x000000ff) ^ rk[10][0];
	tmp[1] = (Te4[state[1] >> 24] & 0xff000000) ^ (Te4[(state[2] >> 16) & 0xff] & 0x00ff0000) ^ (Te4[(state[3] >> 8) & 0xff] & 0x0000ff00) ^ (Te4[state[0] & 0xff] & 0x000000ff) ^ rk[10][1];
	tmp[2] = (Te4[state[2] >> 24] & 0xff000000) ^ (Te4[(state[3] >> 16) & 0xff] & 0x00ff0000) ^ (Te4[(state[0] >> 8) & 0xff] & 0x0000ff00) ^ (Te4[state[1] & 0xff] & 0x000000ff) ^ rk[10][2];
	tmp[3] = (Te4[state[3] >> 24] & 0xff000000) ^ (Te4[(state[0] >> 16) & 0xff] & 0x00ff0000) ^ (Te4[(state[1] >> 8) & 0xff] & 0x0000ff00) ^ (Te4[state[2] & 0xff] & 0x000000ff) ^ rk[10][3];
	State2byte(tmp, ct);
}
